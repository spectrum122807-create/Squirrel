...
def cleanup_old_listings():
    ...
    """)
    # delete messages older than 1 hours
    c.execute("""
    DELETE FROM messages
    WHERE timestamp <= datetime('now', '-60minutes')
""")
...
def dashboard():
    ...
    # get user's active request keywords FIRST
    c.execute("""
        SELECT title, description FROM requests
    ...
    for req in user_requests:
        keywords += req[0].lower().split()
        if req[1]:
            keywords += req[1].lower().split()

    # remove common words
    stopwords = {'a', 'an', 'the', 'i', 'for', 'need', 'want', 'some', 'any', 'is', 'in', 'of', 'to', 'and', 'or'}
    keywords = list(set([k for k in keywords if k not in stopwords and len(k) > 2]))

    # now fetch listings with keyword matching
    if keywords:
        conditions = " OR ".join([
            f"LOWER(listings.title) LIKE '%{k}%' OR LOWER(listings.description) LIKE '%{k}%'"
            for k in keywords
        ])
        c.execute(f"""
            SELECT listings.id, listings.title, listings.price,
                   ...
                   users.room_number,
                   CASE WHEN ({conditions}) THEN 1 ELSE 0 END as is_match
            FROM listings
            JOIN users ON listings.user_id = users.id
            ORDER BY is_match DESC, listings.created_at DESC
            LIMIT 10
        """)
    else:
        c.execute("""
            SELECT listings.id, listings.title, listings.price,
                   listings.description, users.username, listings.status,
                   users.room_number, 0 as is_match
            FROM listings
            JOIN users ON listings.user_id = users.id
...
def want(listing_id):
    ...
    # Check if listing still exists first
    c.execute("SELECT id FROM listings WHERE id=?", (listing_id,))
    listing_exists = c.fetchone()
    if not listing_exists:
        conn.close()
        return redirect(url_for("dashboard"))  # silently redirect
    ...
    "message": f"{session['user']} is interested in your listing!"
}, to=None)
    conn.close()
    return redirect(url_for("dashboard"))
...
def enlist():
    ...
    socketio.emit("refresh", {"message": "update"}, to=None)
    conn.close()
...
def request_page():
    ...
    if request.method == "POST":
        ...
        conn.commit()
        socketio.emit("refresh", {
    "message": f"{session['user']} is looking for something — do you have it?"
}, to=None)
        conn.close()
...
def have(request_id):
    ...
    conn.commit()
    socketio.emit("refresh", {
    "message": f"Someone has what you're looking for!"
}, to=None)
    conn.close()
    ...
    with db_lock:
        ...
        if notif:
            ...
            if buyer:
                ...
                """, (seller_id, listing_title))
                already_exists = c.fetchone()

                if not already_exists:
                    c.execute("""
                        INSERT INTO listing_history (original_id, user_id, title, description, price)
                        SELECT id, user_id, title, description, price
                        FROM listings WHERE title=? AND user_id=?
                    """, (listing_title, seller_id))
                ...
                conn.commit()
                socketio.emit("refresh", {
                     "message": "A deal has been agreed!"
                }, to=None)
        conn.close()
...
def delete_listing(id):
    ...
    with db_lock:
        ...
        # get title first for duplicate check
        c.execute("SELECT title FROM listings WHERE id=?", (id,))
        row = c.fetchone()
        title_to_save = row[0] if row else None
        ...
        c.execute("""
            ...
            WHERE user_id=(SELECT id FROM users WHERE username=?)
            AND title=?
        """, (session["user"], title_to_save))
        already_exists = c.fetchone()
        
        # save to history only if not duplicate
        if not already_exists and title_to_save:
            c.execute("""
                INSERT INTO listing_history (original_id, user_id, title, description, price)
                SELECT id, user_id, title, description, price
                FROM listings WHERE id=? AND user_id=(SELECT id FROM users WHERE username=?)
            """, (id, session["user"]))
...
def delete_request(id):
    ...
    # get all sellers who responded to this request
    c.execute("""
        SELECT users.username FROM availabilities
        JOIN users ON availabilities.seller_id = users.id
        WHERE availabilities.request_id = ?
    """, (id,))
    sellers = c.fetchall()
    ...
    for seller in sellers:
        ...
        c.execute("""
            DELETE FROM messages
            WHERE (sender=? AND receiver=?) OR (sender=? AND receiver=?)
        """, (session["user"], seller_username, seller_username, session["user"]))
...
def history():
    ...
    c.execute("""
        SELECT id, title, description, price, sold_at
...
@app.route("/repost/<int:history_id>")
def repost(history_id):
    if "user" not in session:
        return redirect(url_for("login"))
    with db_lock:
        conn = get_db()
        ...
        user_id = c.fetchone()[0]
        c.execute("""
            SELECT title, description, price FROM listing_history
